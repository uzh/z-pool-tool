...
dev:: ## Run the Sihl app, watch files and restart on change
	...
	echo
	inotifywait -e modify -e move -e create -e delete -e attrib -r `pwd` --exclude "(_build|public|.parcel-cache|logs|Makefile|.vscode|.devcontainer|.git|.DS_Store|node_modules|resources|worker-src)" -qq
	[[ $$(jobs -pr) = "" ]] || kill $$(jobs -pr)
	echo
	done
