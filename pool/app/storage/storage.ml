...
let find label id =
  ...
  match file with
  | None -> raise (Entity.Exception ("File not found with id " ^ id))
  | Some file -> Lwt.return file
;;

let delete label id =
  let%lwt file = find label id in
  let blob_id = file.Entity.blob in
...
let upload_base64 label ?id file base64 =
  ...
  let%lwt blob =
    ...
    | Error (`Msg msg) ->
      Logger.err (fun m -> m "Could not upload base64 content of file %a" pp_file file);
      raise (Entity.Exception msg)
    | Ok blob -> Lwt.return blob
  in
  let%lwt () = Repo.insert_blob label ~id:blob_id blob in
  let stored_file = Entity.{ file; blob = blob_id } in
...
let update_base64 label file base64 =
  ...
  let%lwt blob =
    match Base64.decode base64 with
    | Error (`Msg msg) ->
      Logger.err (fun m -> m "Could not upload base64 content of file %a" pp_stored file);
      raise (Entity.Exception msg)
    | Ok blob -> Lwt.return blob
  in
...
let download_data_base64_opt label file =
  ...
  | Some (Error (`Msg msg)) ->
    Logger.err (fun m -> m "Could not get base64 content of file %a" pp_stored file);
    raise (Entity.Exception msg)
...
let download_data_base64 label file =
  ...
  | None ->
    raise
      (Entity.Exception (Format.asprintf "File data not found for file %a" pp_stored file))
;;
