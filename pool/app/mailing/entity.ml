...
module Start = struct
  ...
  let validate ?(allow_start_in_past = false) start end_at =
    ...
    let* start_at =
      ...
      | StartAt start_at ->
        if Pool_model.Time.is_earlier ~than:(Pool_core.Time.now ()) start_at
  ...
  let create start_at start_now =
    ...
    | false, None -> Error Pool_message.(Error.Conformist [ Field.Start, Error.NoValue ])
  ;;
end
...
module Distribution = struct
  ...
  module SortableField = struct
    module Core = struct
      ...
      type t =
        | AssignmentCount [@name "assignment_count"] [@printer print "assignment_count"]
  ...
  type sorted = (SortableField.t * SortOrder.t) list [@@deriving eq, show, yojson]
  ...
  let schema () =
    ...
    let decode m =
      try m |> Yojson.Safe.from_string |> sorted_of_yojson |> CCResult.return with
  ...
  let of_urlencoded_list =
    ...
    | data ->
      ...
      |> CCList.map (fun distribution_field ->
        ...
        | [ field; order ] -> Ok (Format.asprintf "[[\"%s\"],[\"%s\"]]" field order)
...
let is_past { end_at; _ } = Pool_model.Time.is_later ~than:end_at (Pool_core.Time.now ())
...
let column_invitation_count = (Field.InvitationCount, "invitation_count") |> Column.create
...
let sortable_by =
  searchable_by @ [ column_start; column_end; column_limit; column_invitation_count ]
;;
