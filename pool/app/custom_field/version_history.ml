...
module AnswerRecord = struct
  ...
  type t =
    ...
    | Date of Pool_model.Time.Date.t answer
  ...
  let default_record =
    let make id = { custom_field_uuid = id; value = None; admin_value = None } in
  ...
  let yojson_of_t record =
    ...
    | `List [ `String _; `Assoc answer ] ->
      ...
      |> (function
       | Some (`String uuid) ->
         let answer = CCList.remove_assoc ~eq:CCString.equal field_uuid answer in
  ...
  let from_public (field : Public.t) : t =
    ...
    let make_record (value, admin_value) = { custom_field_uuid; value; admin_value } in
    ...
    in
    match field with
    | Public.Boolean (_, answer) -> extract_answer answer |> make_record |> boolean
    ...
    | Public.MultiSelect (_, _, answer) ->
      ...
      |> make_record
      |> multiselect
    | Public.Number (_, answer) -> extract_answer answer |> make_record |> number
