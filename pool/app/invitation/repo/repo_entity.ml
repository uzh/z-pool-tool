...
type t =
  ...
  ; resent_count : Entity.ResentCount.t
...
let of_entity (experiment_id : Experiment.Id.t) (m : Entity.t) : t =
  ...
  ; resent_count = m.Entity.resent_count
...
let t =
  let encode m =
    Ok
      ...
      , ( m.experiment_id
        , ( m.contact_id
          , (m.resent_at, (m.resent_count, (m.created_at, m.updated_at))) ) ) )
  in
  let decode
    ( id
    , ( experiment_id
      , (contact_id, (resent_at, (resent_count, (created_at, updated_at)))) ) )
    =
...
module Update = struct
  ...
  let t =
    let encode (m : Entity.t) =
      Ok (m.Entity.id, m.Entity.resent_at, m.Entity.resent_count)
    in
    ...
    Caqti_type.(
      custom
        ~encode
        ~decode
        (tup3 Pool_common.Repo.Id.t (option ResentAt.t) ResentCount.t))
  ;;
end
