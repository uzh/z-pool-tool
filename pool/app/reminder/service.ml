...
let send_tenant_reminder database_label =
  ...
  | Error err ->
    Logs.err ~src (fun m ->
      m
        ...
        "Serialized message string was NULL, can not deserialize message. Please fix the \
  ...
  | Ok (email_sessions, text_message_sessions) ->
    let get_ids sessions =
      ...
      |> CCString.concat ", "
    in
    [ email_sessions, "Email reminders"; text_message_sessions, "Text message reminders" ]
