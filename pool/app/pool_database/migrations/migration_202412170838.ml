(* TODO: Use correct email templates *)
let seed_inactive_contact_message_templates2 =
  Database.Migration.Step.create
    ~label:"seed match_filter_update_notification template"
    {sql|
    INSERT INTO `pool_message_templates` (`uuid`, `label`, `language`, `email_subject`, `email_text_html`, `email_text_plain`, `sms_text`) 
      VALUES (
        UNHEX(REPLACE(UUID(), '-', '')), 
        ...
        'Grüezi {name},\r\n\r\nSie haben sich seit dem {lastLogin} nicht mehr in Ihren Account eingeloggt.\r\n\r\nFalls Sie sich bis zum {deactivationAt} nicht einloggen, wird Ihr Accound pausiert und Sie werden an keine weiteren Experimente mehr eingeladen.\r\n\r\nFreundliche Grüsse\r\n{siteTitle}',
        ...
        '<h4>Dear {name},</h4><p>The following registered contacts no longer meet the criteria to participate in the experiment {experimentPublicTitle}:<br><br>{assignments}</p><p>Yours sincerely,<br>{siteTitle}</p>', 
        'Dear {name},\r\n\r\nYou have not logged in to your account since {lastLogin}.\r\n\r\nIf you do not log in by {deactivationAt}, your account will be paused and you will not be invited to any further experiments.\r\n\r\nYours sincerely,\r\n\r\n{siteTitle}',
        'Dear {name},\r\n\r\nYour account will be deactivated on the {deactivationAt}, if you do not log in again.\r\n\r\nYours sincerely,\r\n\r\n{siteTitle}'
      ),(
        UNHEX(REPLACE(UUID(), '-', '')), 
        'inactive_contact_deactivation', 
        'DE', 
        'Ihr Konto wurde aufgrund Inaktivität bald deaktiviert', 
        '<h4>Grüezi {name},</h4><p>Sie haben sich seit dem {lastLogin} nicht mehr in Ihren Account eingeloggt.\r\n\r\nFalls Sie sich bis zum {deactivationAt} nicht einloggen, wird Ihr Accound pausiert und Sie werden an keine weiteren Experimente mehr eingeladen.\r\n\r\nFreundliche Grüsse\r\n{siteTitle}', 
        'Grüezi {name},\r\n\r\nSie haben sich seit dem {lastLogin} nicht mehr in Ihren Account eingeloggt.\r\n\r\nFalls Sie sich bis zum {deactivationAt} nicht einloggen, wird Ihr Accound pausiert und Sie werden an keine weiteren Experimente mehr eingeladen.\r\n\r\nFreundliche Grüsse\r\n{siteTitle}',
        'Grüezi {name},\r\n\r\nIhr Account wird am {deactivationAt} deaktiviert, falls Sie sich nicht mehr einloggen.\r\n\r\nFreundliche Grüsse\r\n{siteTitle}'
      ), (
        UNHEX(REPLACE(UUID(), '-', '')), 
