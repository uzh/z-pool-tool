let seed_admin_account_created_templates =
  Database.Migration.Step.create
    ...
    {sql| INSERT INTO `pool_message_templates` (`uuid`, `label`, `language`, `email_subject`, `email_text_html`, `email_text_plain`, `sms_text`) VALUES (
            ...
            '<div><h4>Liebe*r {name},</h4><p>Für Sie wurde ein Admin-Konto erstellt.<br>Um Ihr Konto zu aktivieren und Ihr Passwort zu setzen, verifizieren Sie bitte Ihre E-Mail-Adresse. Klicken Sie dazu auf diesen <a href="{verificationUrl}"> Link</a>.</p><p>Nach erfolgreicher Verifizierung senden wir Ihnen eine zweite E-Mail mit einem Link zum Setzen Ihres Passworts zu.</p><p>Wenn Sie diese Aktion nicht durchgeführt haben, bitten wir Sie, diese E-Mail zu ignorieren oder uns zu kontaktieren.</p><p>Sollte der Link nicht funktioniert, kopieren Sie bitte den folgenden Link manuell in Ihren Browser: {verificationUrl}</p><p>Freundliche Grüsse,<br>{siteTitle}</p></div>',
