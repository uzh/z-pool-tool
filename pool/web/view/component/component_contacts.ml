...
let status_icons { Contact.paused; email_verified; verified; _ } =
  (* TODO: Add SMTP Bounce *)
  (* TODO: Define which icon to use when email unverified *)
  let open Pool_user in
  let open Component_icon in
  [ paused |> Paused.value, NotificationsOffOutline
  ; CCOption.is_some email_verified, MailOutline
...
let status_icons_table_legend language =
  ...
  ; ( field_to_string Message.Field.EmailAddressVerified
    , legend_icon_item MailOutline )
...
let identity_with_icons view_contact_name contact =
  let text =
    if view_contact_name
    then Contact.lastname_firstname contact
  ...
  |> span ~a:[ a_class [ "flexrow"; "flex-gap-sm" ] ]
;;
