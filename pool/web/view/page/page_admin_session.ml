...
let follow_up_icon language =
  span
    ...
    [ abbr
        ~a:
          [ a_title
              (Pool_common.Utils.field_to_string language Field.FollowUpSession)
          ]
        [ txt "(F)" ]
    ]
;;
  ...
  let chronological_toggle language chronological =
    ...
    then
      ...
      div
        ...
        ; a
            ~a:
              [ a_href (Format.asprintf "?%s=true" Field.(show Chronological)) ]
...
let reschedule_session
  ...
  |> Layout.Experiment.(
       create
         context
         (Control Control.(Reschedule (Some Field.Session)))
         experiment)
;;
...
let duplicate
      ...
      [ txt
          Pool_common.(
            Utils.control_to_string
              language
              (Control.Create (Some Field.Sessions)))
      ]
  in
  let form =
    form
  ...
  |> Layout.Experiment.(
       create
         context
         (Control Control.(Duplicate (Some Field.Session)))
         experiment)
;;
...
let follow_up
  ...
  |> Layout.Experiment.(
       create
         context
         (Control Control.(Create (Some Field.FollowUpSession)))
         experiment)
;;
