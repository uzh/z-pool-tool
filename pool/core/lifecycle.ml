(* Extracted from the Sihl web framework (https://github.com/oxidizing/sihl), MIT license.
   Source:
   https://github.com/uzh/sihl/blob/6c3c4040413294155cda3a363edf0ff3c7e638b8/sihl/src/core_lifecycle.ml *)

let log_src = Logs.Src.create "pool.core.container"
...
let create_lifecycle
  ...
  counter := !counter + 1;
  let implementation_name = Option.value implementation_name ~default:type_name in
...
let top_sort_lifecycles lifecycles =
  ...
  let lifecycle_graph =
    ...
    |> List.map (fun (id, lifecycle) ->
      let dependencies = lifecycle.dependencies () |> List.map (fun dep -> dep.id) in
      id, dependencies)
  in
  Logger.debug (fun m ->
    m
  ...
  | Tsort.Sorted sorted ->
    Logger.debug (fun m -> m "Pre sorted lifecycle graph: %s" ([%show: int list] sorted));
    sorted
    |> List.map (fun id ->
      ...
      | None ->
        Logger.err (fun m ->
          m
            "Failed to sort lifecycles. Lifecycce id %d not found in registered \
             lifecycles: %a"
            id
            pp_map
            lifecycles);
        Logger.info (fun m ->
          m
            "It looks like a service or command is depending on a service that has not \
  ...
  | Tsort.ErrorCycle remaining_ids ->
    ...
    let remaining_msg =
      Option.map
        ...
        remaining_names
    in
    Logger.err (fun m -> m "%s" @@ Option.value remaining_msg ~default:msg);
    raise Exception
;;
