(* Extracted from the Sihl web framework (https://github.com/oxidizing/sihl), MIT license.
   Source:
   https://github.com/uzh/sihl/blob/6c3c4040413294155cda3a363edf0ff3c7e638b8/sihl/src/core_command.ml *)

let log_src = Logs.Src.create "pool.core.command"
...
type t =
  ...
  ; usage : string option
  ; description : string
  ; dependencies : Lifecycle.lifecycle list [@opaque]
...
let run commands args =
  ...
  match command with
  | Some command ->
    (* We use the first argument to find the command, the command itself receives the
       rest arguments. *)
    let rest_args =
    ...
    (match rest_args with
     ...
     | rest_args ->
       ...
       Lwt.catch
         (fun () ->
            let%lwt _ =
              Lwt_list.iter_s (fun (lifecycle : Lifecycle.lifecycle) ->
                lifecycle.Lifecycle.start ())
              @@ Lifecycle.top_sort_lifecycles command.dependencies
            in
