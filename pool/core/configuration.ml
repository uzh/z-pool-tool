(* Extracted from the Sihl web framework (https://github.com/oxidizing/sihl), MIT license.
   Source:
   https://github.com/uzh/sihl/blob/6c3c4040413294155cda3a363edf0ff3c7e638b8/sihl/src/core_configuration.ml
   The unused [read_int] function has been removed. The SIHL_* environment variable
   names are kept for backwards compatibility with existing deployments. *)

let log_src = Logs.Src.create "pool.core.configuration"
...
let make ?(schema : ('ctor, 'ty) schema option) () =
  match schema with
  | Some schema ->
    Conformist.fold_left
      ~f:(fun res field ->
        let name = Conformist.Field.name field in
        let description = Option.value ~default:"-" (Conformist.Field.meta field) in
        let type_ = Conformist.Field.type_ field in
        let default = Conformist.Field.encode_default field in
        List.cons { name; description; type_; default = CCList.head_opt default } res)
      ~init:[]
      schema
  | None -> []
;;
...
let read_env_file () =
  ...
  | Some path ->
    ...
    if exists
    then (
      Logger.info (fun m -> m "Env file found: %s" filename);
      let envs = CCIO.read_lines_l (open_in filename) in
      Some (envs_to_kv envs))
    else (
      Logger.info (fun m -> m "Env file not found: %s. Continuing without it." filename);
      None)
  | None ->
    Logger.debug (fun m ->
      m
...
let read (schema : ('ctor, 'ty) schema) : 'ty =
    ...
    let errors =
      List.map
        (fun (field, input, msg) ->
           ...
           | None -> Format.sprintf "Failed to read configuration '%s': %s" field msg
           | Some input ->
             Format.sprintf
        ...
        errors
    in
    List.iter (fun error -> Logger.err (fun m -> m "%s" error)) errors;
...
let load () =
  ...
  | None ->
    Logger.info (fun m ->
      m
        "SIHL_ENV not found. Set it to one of the following values: development, \
         production, test");
    failwith "SIHL_ENV not found"
  | Some env -> Logger.info (fun m -> m "SIHL_ENV: %s" env)
;;
...
let read_secret () =
  ...
  | true, Some secret ->
    ...
    then secret
      ...
      Logger.err (fun m -> m "SIHL_SECRET has to be longer than 10");
      failwith "Insecure secret provided")
  | false, Some secret -> secret
  | true, None ->
    Logger.err (fun m -> m "Set SIHL_SECRET before deploying to production");
...
let require (schema : ('ctor, 'ty) schema) = read schema |> ignore
...
let print_cmd (configurations : t list) : Command.t =
  Command.make
    ~name:"config"
    ~description:"Prints a list of known configurations."
    (fun _ ->
       configurations
