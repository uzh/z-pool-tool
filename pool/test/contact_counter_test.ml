...
module UpdateAssignments = struct
  ...
  let update_assignment_manually =
    Test_utils.case
    @@ fun () ->
    let%lwt contact, experiment, session, _ = get_entities () in
  ...
  let update_follow_up_assignment_manually =
    Test_utils.case
    @@ fun () ->
