...
let create_invitations_repo _ () =
  ...
  let mailing () =
    ...
    in
    create Start.StartNow (end_time ()) rate distribution |> get_or_failwith
  in
  ...
  >|> Lwt_list.iter_s
        (fun ({ Mailing.rate; process; _ } as mailing : Mailing.t) ->
           ...
           | NewInvitation ->
             ...
             let expected =
               expected_events experiment contacts create_message
             in
             let%lwt events =
               Matcher.match_invitation_events ~interval [ pool ]
             in
             let events =
               CCList.assoc ~eq:Pool_database.Label.equal pool events
               |> sort_events
             in
             ...
             let () =
               let msg =
                 "count generated invitations -> smaller or equal rate"
               in
               let is_less_or_equal =
                 after - before <= Mailing.Rate.value rate
               in
               Alcotest.(check bool msg true is_less_or_equal)
             in
             Lwt.return_unit
           | ResendOnce | ResendTwice -> failwith "Unhandled")
;;
