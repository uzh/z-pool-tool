...
let create_operator_without_password () =
  let open Data in
  let id = Admin.Id.create () in
  let password = "GeneratedPassword99!" |> Pool_user.Password.Plain.create in
  let events =
    let open CCResult.Infix in
    let open Admin_command.CreateOperator in
    (* the password is not part of the form data but generated by the handler *)
    [ Field.Email, [ email ]
    ; Field.Firstname, [ firstname ]
    ; Field.Lastname, [ lastname ]
    ]
    |> CCList.map (fun (k, v) -> Field.show k, v)
    |> decode
    >>= handle ~id ~password
  in
  let expected =
