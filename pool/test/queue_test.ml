...
module DlrData = struct
  ...
  let submit_date = Time.Parsing.parse_date_time submit_date_str |> CCResult.get_exn
  let done_date_str = "2024-06-24T10:27:56Z"
  let done_date = Time.Parsing.parse_date_time done_date_str |> CCResult.get_exn
  let plmn = "22801"
  let country = "ch"
  let sms_cost = 0.1
