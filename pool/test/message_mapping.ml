...
let check_message_template ?label =
  ...
  let valid_label = Pool_common.MessageTemplateLabel.(Alcotest.testable pp equal) in
  Email.message_template %> check (option valid_label) "correct message template" label
;;

let check_mapped_uuids expected =
  let open Alcotest in
  let valid_history = History.(Alcotest.testable pp_item equal_item) in
  Email.job_ctx
  %> function
  | Some (Create mapping_uuids) ->
    check (list valid_history) "correct mapped ids" (History.sort expected) mapping_uuids
  | Some (Clone _) | None ->
...
let account_suspension_notification _ () =
  ...
  in
  let () = check_message_template ~label:AccountSuspensionNotification res in
...
let assignment_confirmation _ () =
  ...
  in
  let () = check_message_template ~label:AssignmentConfirmation res in
  let () =
    check_mapped_uuids
      JobHistory.
        [ experiment_item experiment; session_item session; contact_item contact ]
      res
  in
  Lwt.return_unit
;;
...
let assignment_session_change _ () =
  ...
  in
  let () = check_message_template ~label:AssignmentSessionChange res in
  let () = check_mapped_uuids expected_uuids res in
  Lwt.return_unit
;;
...
let contact_email_change_attempt _ () =
  ...
  in
  let () = check_message_template ~label:ContactEmailChangeAttempt res in
  let () = check_mapped_uuids History.[ User, Contact.Id.to_common contact_id ] res in
  Lwt.return_unit
;;
...
let contact_registration_attempt _ () =
  ...
  in
  let () = check_message_template ~label:ContactRegistrationAttempt res in
  let () = check_mapped_uuids History.[ User, Contact.Id.to_common contact_id ] res in
  Lwt.return_unit
;;
...
let email_verification _ () =
  ...
  in
  let expected = EmailVerification, [ Contact.Id.to_common contact_id ] in
...
let experiment_invitation _ () =
  ...
  let () = check_message_template ~label:ExperimentInvitation res in
  let () =
    check_mapped_uuids
      History.
        [ Experiment, Experiment.Id.to_common experiment_id
...
let password_change _ () =
  ...
  let () = check_message_template ~label:PasswordChange res in
  let () = check_mapped_uuids [ History.User, Contact.Id.to_common contact_id ] res in
  Lwt.return_unit
;;
...
let phone_verification _ () =
  ...
  let expected =
    Text_message.create_job
      ~message_template:PhoneVerification
...
let session_reminder _ () =
  ...
  let expected =
    Text_message.create_job
      ~message_template:expected_label
      ~job_ctx:(job_ctx_create expected_uuids)
...
module Resend = struct
  ...
  let resend_pending () =
    let email_job = email_queue_job ~status:Status.Pending () in
  ...
  let resend_unchanged () =
    ...
    let expected =
      Ok
        [ Model.create_email_job ~job_ctx:(job_ctx_clone (email_job |> Instance.id)) ()
          |> Email.sent
          |> Pool_event.email
        ]
    in
    ...
    let expected =
      Ok
        [ Model.create_text_message_job
            ~job_ctx:(Clone (text_message_job |> Instance.id))
            cell_phone
          |> Text_message.sent
  ...
  let resend_updated_recipient () =
    ...
    let expected =
      Ok
        [ Model.create_email_job ~job_ctx:(Clone (email_job |> Instance.id)) ()
