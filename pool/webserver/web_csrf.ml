(* Extracted from the Sihl web framework (https://github.com/oxidizing/sihl), MIT license.
   Source:
   https://github.com/uzh/sihl/blob/6c3c4040413294155cda3a363edf0ff3c7e638b8/sihl/src/web_csrf.ml
   The unused [find] function has been removed. *)

let log_src = Logs.Src.create "pool.middleware.csrf"
...
let find_exn req =
  ...
  | Some csrf -> csrf
  | None -> failwith "CSRF token was fetched but CSRF middleware is not installed"
;;
...
module Crypto = struct
  ...
  module Secret : sig
    ...
    (** [make raw] turns a [raw] string secret to a fixed length SHA256 digest. A fixed
        length secret is required for the hard-coded AES key sizes to work. *)
    val make : string -> t
  ...
  module Encrypted_token : sig
    ...
    (** [to_uri_safe_string tkn] turns an encrypted token [tkn] into a URI-safe base64
        string. Used to make sure CSRF works with all encodings. *)
    val to_uri_safe_string : t -> string

    (** [of_uri_safe_string tkn] turns a URI-safe string [tkn] into an encrypted token.
        Attempts to parse the base64 input [tkn], which can fail. This does not perform
        any cryptographic operation to ensure [tkn] can be decrypted. *)
    ...
    (** [from_struct ~with_secret tkn] encrypts a raw token [tkn] using AES in ECB mode
        given a secret [with_secret]. *)
    val from_struct : with_secret:Secret.t -> Cstruct.t -> t

    (** [from_struct_random ~with_secret tkn] encrypts a raw token [tkn]. Additionally
        the encrypted result is scrambled with a random salt (IV) using AES in CBC mode
        given a secret [with_secret].*)
    ...
    let to_uri_safe_string t =
      t |> Cstruct.to_string |> Base64.encode_string ~alphabet:Base64.uri_safe_alphabet
    ;;
  ...
  (** This module does not provide an API to read a decrypted token (by turning it into
      a string, Cstruct.t or similar). This is to prevent leaking CSRF tokens. *)
  module Decrypted_token : sig
    ...
    (** [from_encrypted ~with_secret tkn] decrypts an encrypted token [tkn] using AES in
        ECB mode given a secret [with_secret]. *)
    val from_encrypted : with_secret:Secret.t -> Encrypted_token.t -> t

    (** [from_encrypted_random ~with_secret tkn] decrypts a randomized encrypted token
        [tkn] given a secret [with_secret]. This function reverses
        [Encrypted_token.from_struct_random] since a specific format is required. *)
    val from_encrypted_random : with_secret:Secret.t -> Encrypted_token.t -> t

    (** [from_encrypted_to_encrypted_random ~with_secret tkn] turns a normal encrypted
        token [tkn] into a randomly encrypted token by first decrypting it and then
        re-encrypting it with [Encrypted_token.from_struct_random].*)
    ...
    let from_encrypted_random ~with_secret (value : Encrypted_token.t) : Cstruct.t =
      let open Cstruct in
      let open Mirage_crypto.AES.CBC in
      let key = with_secret |> Secret.to_raw |> to_string |> of_secret in
      let iv, value =
        value |> Encrypted_token.to_struct |> CCFun.flip Cstruct.split block_size
      in
      decrypt ~key ~iv:(to_string iv) (to_string value) |> of_string
    ;;

    let from_encrypted_to_encrypted_random ~with_secret value =
      from_encrypted ~with_secret value |> Encrypted_token.from_struct_random ~with_secret
    ;;
  end
end
...
let middleware
      ...
      ?(input_name = "_csrf")
      ?(secret = Pool_core.Configuration.read_secret ())
  ...
  let filter handler req =
    let check_csrf =
      ...
      || Option.value (Pool_core.Configuration.read_bool "CHECK_CSRF") ~default:false
    in
    if not check_csrf
    then
      ...
      in
      (* Create a new randomized, decryptable token for each request to mitigate BREACH
         attack *)
      ...
      in
      let req =
        set (submittable_encrypted_token |> Encrypted_token.to_uri_safe_string) req
      in
      let construct_response handler =
        ...
        |> Lwt.map
           @@ fun resp ->
           (* Try to add csrf to session, if it does not exist, make a new session *)
        ...
        (match stored_encrypted_token with
         ...
         | Some stored_encrypted_token ->
           ...
           then construct_response handler
             ...
             Logs.err (fun m ->
               m
                 "Encrypted stored token '%s' does not match with the received encrypted \
                  token '%s'"
