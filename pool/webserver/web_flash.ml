(* Extracted from the Sihl web framework (https://github.com/oxidizing/sihl), MIT license.
   Source:
   https://github.com/uzh/sihl/blob/6c3c4040413294155cda3a363edf0ff3c7e638b8/sihl/src/web_flash.ml
   The unused [find_notice] and [set_notice] functions have been removed. *)

let log_src = Logs.Src.create "pool.middleware.flash"
...
module Env = struct
  let key : Flash.t Opium.Context.key = Opium.Context.Key.create ("flash", Flash.sexp_of_t)
end

let find' req = Opium.Context.find Env.key req.Opium.Request.env
let find_alert req = Option.bind (find' req) (fun flash -> flash.Flash.alert)
  ...
  | Some cookie_value ->
    (match Flash.of_json cookie_value with
     | None ->
       ...
       Logs.info (fun m ->
         m
           "Maybe the cookie key '%s' collides with a cookie issued by someone else. Try \
            to change the cookie key."
           cookie_key);
       Parse_error
    ...
    (match old_flash with
     | Some old_flash ->
       ...
       then (* Same flash value, don't set cookie *)
         ...
         let resp =
           Opium.Response.add_cookie_or_replace ~scope:(Uri.of_string "/") cookie resp
         in
         resp)
     | None ->
...
let middleware ?(cookie_key = "_flash") () =
  let filter handler req =
    ...
    | Found flash ->
      ...
      let%lwt resp = handler req in
      Lwt.return @@ persist_flash ~delete_if_not_set:true ~old_flash:flash cookie_key resp
  in
