(* Extracted from the Sihl web framework (https://github.com/oxidizing/sihl), MIT license.
   Sources:
   https://github.com/uzh/sihl/blob/6c3c4040413294155cda3a363edf0ff3c7e638b8/sihl/src/web_http.ml
   https://github.com/uzh/sihl/blob/6c3c4040413294155cda3a363edf0ff3c7e638b8/sihl/src/contract_http.ml
   The service signature from [contract_http.ml] has been removed; only its [name] is
   kept. *)
...
let routers_to_opium_builders routers =
  ...
  |> List.map (fun router ->
    ...
    routes
    |> List.map (fun (meth, route, handler) -> meth, Web.externalize_path route, handler)
    |> List.map to_opium_builder
    |> List.rev)
  |> List.concat
;;

let log_src = Logs.Src.create ("pool.service." ^ name)
...
let schema =
  ...
  make
    [ optional ~meta:"The port the HTTP server listens on." (int ~default:3000 "PORT") ]
    config
;;
...
let start_server () =
  ...
  let port_nr = Option.value (Pool_core.Configuration.read schema).port ~default:33000 in
  let app =
    Opium.App.(
      empty
      |> port port_nr
      |> cmd_name "Z-Pool-Tool"
      |> fun app ->
  ...
  (* Registered middlewares need to be mounted before routing happens, so that every
     single request goes through them, not only requests that match handlers *)
...
let routes_cmd =
  Pool_core.Command.make ~name:"routes" ~description:"Prints all HTTP routes" (fun _ ->
    !registered_router
...
let lifecycle = Pool_core.Container.create_lifecycle "http" ~start ~stop
