(* Extracted from the Sihl web framework (https://github.com/oxidizing/sihl), MIT license.
   Source:
   https://github.com/uzh/sihl/blob/6c3c4040413294155cda3a363edf0ff3c7e638b8/sihl/src/web_session.ml
   The unused [get_all] function has been removed. *)

let log_src = Logs.Src.create "pool.middleware.session"
...
let update_or_set_value
  ...
  =
  let signed_with = Opium.Cookie.Signer.make secret in
  let mreq = decode_session_req cookie_key signed_with req |> CCResult.get_or_failwith in
  let mresp =
