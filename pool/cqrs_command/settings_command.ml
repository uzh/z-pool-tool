...
module UpdatePhoneVerificationEnabled : sig
  type t = Settings.PhoneVerification.t
  ...
  let handle ?(tags = Logs.Tag.empty) enabled =
    Logs.info ~src (fun m -> m "Handle command UpdatePhoneVerificationEnabled" ~tags);
    Ok [ Settings.PhoneVerificationEnabledUpdated enabled |> Pool_event.settings ]
  ;;

  let schema = Conformist.(make Field.[ Settings.PhoneVerification.schema () ] CCFun.id)
