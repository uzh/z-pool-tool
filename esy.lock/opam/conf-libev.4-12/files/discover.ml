...
let search_header header =
  let rec loop = function
    | [] -> None
    | dir :: dirs ->
      if Sys.file_exists (dir ^ "/include/" ^ header)
      then Some dir
      else loop dirs
  in
  loop search_paths
...
let () =
  ...
  at_exit (fun () ->
    (try close_out config with
     | _ -> ());
    (try close_out config_ml with
     | _ -> ());
  ...
  let missing = [] in
  let missing =
    if test_feature "libev" "HAVE_LIBEV" ~args:"-cclib -lev" libev_code
    then missing
    else "libev" :: missing
  in
  if missing <> []
...
(* ignore (test_feature "eventfd" "HAVE_EVENTFD" eventfd_code); ignore
   (test_feature "fd passing" "HAVE_FD_PASSING" fd_passing_code); ignore
   (test_feature "sched_getcpu" "HAVE_GETCPU" getcpu_code); ignore (test_feature
   "affinity getting/setting" "HAVE_AFFINITY" affinity_code); ignore
   (test_feature "credentials getting" "HAVE_GET_CREDENTIALS"
   get_credentials_code); ignore (test_feature "fdatasync" "HAVE_FDATASYNC"
   fdatasync_code) *)
